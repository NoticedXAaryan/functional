...
with sync_playwright() as playwright:
    ...
    preparer.get_by_label('Verified missing-child case').select_option(receipt['case_id'])
    preparer.get_by_label('Registered audience area').select_option('demo-nagar-north')
    preparer.get_by_label('Public description').fill(description)
    preparer.get_by_label('Issuing organization name').fill('[FICTIONAL] Browser Test Team')
    preparer.get_by_label('Verification reference').fill('Synthetic fixture reviewed for demonstration')
    preparer.get_by_label('Expiry in your local time').fill((datetime.now() + timedelta(hours=2)).strftime('%Y-%m-%dT%H:%M'))
    ...
    card.get_by_role('button', name='Approve revision').click()
    expect(approver.get_by_role('status')).to_contain_text('Revision approved')
    ...
    public.get_by_label('View area').select_option('demo-nagar-north')
    ...
    alert.get_by_role('link', name='View current status').click()
    public.wait_for_load_state('networkidle')
