# S07 · Ask another team for help

Target route: `/cases/{id}/transfer`. Layout: **staff_list_detail**. Design only.
...
3. Heading: **Ask another team for help**.
4. Supporting text: “Keep responsibility until the receiving team accepts.”
5. Authorized receiving team.
6. Reason.
7. Minimum necessary summary.
8. Primary button: **Request transfer**.
9. Secondary actions: Cancel → S03.
10. Footer: Other ways to get help, Privacy, Staff sign in (public screens only).

## Required behaviour

Primary action contract: `create_transfer_and_show_pending_ack`. Required capability: `authorized_transfer_targets`. Replace braced values only with authorized API data. If required real data is absent, show a truthful unavailable state; never display the braces or invent a value.
...
Use tokens.css and the corresponding visual board. The board is composition guidance only; it is not a screenshot of a working service. Consult ../.. documents 02–06 for domain-specific validation and authorization. For S07, use the exact ID in the release checklist.
