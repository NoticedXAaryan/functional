# S05 · Accept this request

Target route: `/assignments/{id}/accept`. Layout: **staff_list_detail**. Design only.
...
3. Heading: **Accept this request**.
4. Supporting text: “Your acceptance tells the team you are taking responsibility.”
5. Case and assignment summary.
6. Primary button: **Accept request**.
7. Secondary actions: Back → S03.
8. Footer: Other ways to get help, Privacy, Staff sign in (public screens only).

## Required behaviour

Primary action contract: `atomic_acceptance_then:S06`. Required capability: `current_assignment`. Replace braced values only with authorized API data. If required real data is absent, show a truthful unavailable state; never display the braces or invent a value.
...
Use tokens.css and the corresponding visual board. The board is composition guidance only; it is not a screenshot of a working service. Consult ../.. documents 02–06 for domain-specific validation and authorization. For S05, use the exact ID in the release checklist.
