...
Target route: `/cases`. Layout: **staff_list_detail**. Design only.
...
3. Heading: **My work**.
4. Supporting text: “Open a request to see what needs attention.”
5. New.
6. Waiting for me.
7. Waiting for child.
8. Escalated.
9. Authorized queue from API.
10. Primary button: **Open request**.
11. Secondary actions: Alert review → S10.
12. Footer: Other ways to get help, Privacy, Staff sign in (public screens only).

## Required behaviour

Primary action contract: `select_authorized_case_then:S03`. Required capability: `staff_case_scope`. Replace braced values only with authorized API data. If required real data is absent, show a truthful unavailable state; never display the braces or invent a value.
...
Use tokens.css and the corresponding visual board. The board is composition guidance only; it is not a screenshot of a working service. Consult ../.. documents 02–06 for domain-specific validation and authorization. For S02, use the exact ID in the release checklist.

## Screen-specific rules override the generic recipe

The Open request action lives on each actual authorized queue item, not an orphan footer button with no selected case. Empty queue has no invented request.
