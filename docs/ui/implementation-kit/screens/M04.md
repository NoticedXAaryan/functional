# M04 · Check what you are sending

Target route: `/missing/review`. Layout: **child_single_column**. Design only.
...
4. Supporting text: “The team reviews this before sharing an alert.”
5. Your message — Change.
6. Place and time — Change.
7. Receiving team.
8. Primary button: **Send privately**.
9. Secondary actions: Back → M03.
10. Footer: Other ways to get help, Privacy, Staff sign in (public screens only).

## Required behaviour

Primary action contract: `commit_private_missing_report_then:M05`. Required capability: `private_case_commit`. Replace braced values only with authorized API data. If required real data is absent, show a truthful unavailable state; never display the braces or invent a value.
...
Use tokens.css and the corresponding visual board. The board is composition guidance only; it is not a screenshot of a working service. Consult ../.. documents 02–06 for domain-specific validation and authorization. For M04, use the exact ID in the release checklist.
