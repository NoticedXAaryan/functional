# S04 · Choose a responder

Target route: `/cases/{id}/assign`. Layout: **staff_list_detail**. Design only.
...
3. Heading: **Choose a responder**.
4. Supporting text: “Choose someone who can take responsibility.”
5. Eligible active responders from API.
6. Primary button: **Assign request**.
7. Secondary actions: Cancel → S03.
8. Footer: Other ways to get help, Privacy, Staff sign in (public screens only).

## Required behaviour

Primary action contract: `atomic_assignment_then:S03`. Required capability: `supervisor_scope`. Replace braced values only with authorized API data. If required real data is absent, show a truthful unavailable state; never display the braces or invent a value.
...
Use tokens.css and the corresponding visual board. The board is composition guidance only; it is not a screenshot of a working service. Consult ../.. documents 02–06 for domain-specific validation and authorization. For S04, use the exact ID in the release checklist.
