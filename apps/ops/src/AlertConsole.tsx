...
export default function AlertConsole({ staff }: { staff: StaffClaims }) {
 ...
 const [tips,setTips]=useState<Record<string,Array<{id:string;sighting_description:string;approximate_location?:string;received_at:string}>>>({});
  ...
  return <section>
    ...
    {revisions.map(d => <article className="ops-card ops-alert-card" key={d.id}><div className="ops-detail-header"><h3 className="ops-title">{d.area_name}</h3><span className="ops-badge">{d.alert_status}{d.is_test ? ' · TEST / FICTIONAL' : ''}</span></div><p className="ops-alert-body">{d.description_text}</p><p>Issuer: {d.issuer_name}</p><p>Verification reference: {d.verification_reference || 'Missing — cannot publish'}</p><p>Expires: {new Date(d.expiry_at).toLocaleString()}</p><p className="ops-muted">Queued: {d.queued} · Accepted by notification service: {d.provider_accepted} · Not sent / unconfirmed: {d.failed}</p>
      {close && <button className="ops-btn-secondary" onClick={async()=>{try{const response=await api(`/staff/tips?alert_id=${d.alert_id}`);setTips(old=>({...old,[d.alert_id]:response.tips}))}catch(e){setError((e as Error).message)}}}>Read private tips</button>}
      ...
      {tips[d.alert_id] && <section className="ops-help" aria-label="Private tips"><h4>Private tips</h4>{tips[d.alert_id].length===0 ? <p>No tips received.</p> : tips[d.alert_id].map(t=><article key={t.id} style={{padding:'12px 0',whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}><p>{t.sighting_description}</p><p>{t.approximate_location}</p><small>{new Date(t.received_at).toLocaleString()}</small></article>)}</section>}
    </article>)}
