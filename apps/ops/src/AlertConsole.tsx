...
export default function AlertConsole({ staff }: { staff: StaffClaims }) {
  ...
  return <section>
    ...
    <p className="ops-subtitle">Notification mode: <strong>{mode || 'checking'}</strong>. Provider acceptance does not prove device delivery.</p>
    ...
    {prepare && <form className="ops-card ops-form" onSubmit={async e => {
      e.preventDefault();
      if (await act('/staff/alert-drafts', { ...draft, expiry_at: new Date(draft.expiry_at).toISOString() }, 'Draft saved for independent review. Nothing has been sent.')) setDraft({ case_id: '', area_id: '', description_text: '', issuer_name: '', verification_reference: '', expiry_at: '' });
      ...
      <label className="ops-label" htmlFor="alert-case">Verified missing-child case</label><select id="alert-case" className="ops-input" required value={draft.case_id} onChange={e => setDraft({ ...draft, case_id: e.target.value })}><option value="">Choose case</option>{cases.map(c => <option key={c.case_id ?? c.id} value={c.case_id ?? c.id}>{c.case_id ?? c.id}</option>)}</select>
      <label className="ops-label" htmlFor="alert-area">Registered audience area</label><select id="alert-area" className="ops-input" required value={draft.area_id} onChange={e => setDraft({ ...draft, area_id: e.target.value })}><option value="">Choose area</option>{areas.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
      ...
      <label className="ops-label" htmlFor="alert-issuer">Issuing organization name</label><input id="alert-issuer" className="ops-input" required maxLength={255} value={draft.issuer_name} onChange={e => setDraft({ ...draft, issuer_name: e.target.value })}/>
      <label className="ops-label" htmlFor="alert-reference">Verification reference (staff only)</label><input id="alert-reference" className="ops-input" required maxLength={500} value={draft.verification_reference} onChange={e => setDraft({ ...draft, verification_reference: e.target.value })}/>
      <label className="ops-label" htmlFor="alert-expiry">Expiry in your local time (within 24 hours)</label><input id="alert-expiry" className="ops-input" type="datetime-local" required value={draft.expiry_at} onChange={e => setDraft({ ...draft, expiry_at: e.target.value })}/>
      <button className="ops-btn-primary" disabled={busy}>Save for review</button>
    </form>}
    {revisions.length === 0 && <p className="ops-empty">No alert revisions in your organization.</p>}
    {revisions.map(d => <article className="ops-card ops-alert-card" key={d.id}><div className="ops-detail-header"><h3 className="ops-title">{d.area_name}</h3><span className="ops-badge">{d.alert_status}{d.is_test ? ' · TEST / FICTIONAL' : ''}</span></div><p className="ops-alert-body">{d.description_text}</p><p>Issuer: {d.issuer_name}</p><p>Verification reference: {d.verification_reference || 'Missing — cannot publish'}</p><p>Expires: {new Date(d.expiry_at).toLocaleString()}</p><p className="ops-muted">Queued: {d.queued} · Provider accepted: {d.provider_accepted} · Failed / uncertain: {d.failed}</p>
      <div className="ops-alert-actions">
        {approve && d.status === 'DRAFT' && d.alert_status === 'DRAFT' && <><button className="ops-btn-primary" disabled={busy || d.prepared_by === staff.staffId} onClick={() => void act('/staff/alert-approvals', { revision_id: d.id, decision: 'approve' }, 'Revision approved. Publish is a separate action.')}>Approve revision</button><button className="ops-btn-secondary" disabled={busy || d.prepared_by === staff.staffId} onClick={() => void act('/staff/alert-approvals', { revision_id: d.id, decision: 'reject' }, 'Revision rejected. Prepare a new draft with corrected information.')}>Reject</button></>}
