...
export default function CommunityAlerts() {
  ...
  return <div className="savera">
    <header className="sa-header"><a className="sa-brand" href="/alerts"><img src="/savera.svg" alt="" width="46" height="46"/><span>Savera <strong>Alert</strong><small>Local awareness. Accountable action.</small></span></a><a className="sa-help" href="/">Ask for private help ↗</a></header>
    {config?.is_test && <div className="sa-test">TEST ENVIRONMENT · Fictional alerts only · Invited adult testers</div>}
    <main className="sa-main">
      <section className="sa-intro"><p className="sa-eyebrow">A community that pays attention</p><h1>{alertID ? 'One alert.\nThe current facts.' : 'The right people.\nThe right area.'}</h1><p>Receive approved missing-child alerts for an area you choose. Share what you observed privately with the reviewing team.</p><div className="sa-principles"><span>Reviewed before publication</span><span>No continuous location tracking</span><span>Stop notifications any time</span></div></section>
      ...
      <div className="sa-layout"><section className="sa-feed" aria-label="Current alerts"><div className="sa-section-heading"><h2>{alertID ? 'Current status' : 'Area bulletin'}</h2><button onClick={() => void refresh()}>Refresh</button></div>
        {!alertID && <label className="sa-filter">View area<select value={area} onChange={e => setArea(e.target.value)}><option value="">All registered areas</option>{config?.areas.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></label>}
        {!loaded && !error && <p role="status">Checking current alerts…</p>}
        {loaded && alerts.length === 0 && <div className="sa-empty"><img src="/savera.svg" width="56" height="56" alt=""/><h3>No active alerts here</h3><p>There are no current published alerts in this view. This is not a guarantee that nobody is missing.</p></div>}
        {alerts.map(a => <article className="sa-alert" key={a.alert_id}><div className="sa-alert-top"><span className={`sa-state ${a.status === 'ACTIVE' ? 'sa-active' : ''}`}>{a.status === 'ACTIVE' ? 'Active alert' : a.status.toLowerCase()}</span>{a.is_test && <span className="sa-fictional">TEST · FICTIONAL</span>}</div><h3>{a.area_name}</h3>{a.status === 'ACTIVE' ? <p className="sa-description">{a.description_text}</p> : <p>This alert is no longer active. Identifying details have been removed. Do not continue circulating an older copy.</p>}<dl><div><dt>Published by</dt><dd>{a.issuer_name}</dd></div><div><dt>Expires</dt><dd>{new Date(a.expiry_at).toLocaleString()}</dd></div></dl>{!alertID && <a className="sa-button" href={`/alerts/${a.alert_id}`}>View current status →</a>}</article>)}
        {alertID && alerts[0]?.tip_route_available && <section className="sa-tip"><h2>Did you notice something?</h2><p>Describe only what you observed. Do not approach, follow or confront anyone. Your tip is private to the authorized reviewing team.</p>{tipReceipt ? <div className="sa-notice" role="status">Tip received. Keep this receipt: <code>{tipReceipt}</code>. This confirms receipt, not that someone has reviewed it.</div> : <form onSubmit={submitTip}><label>What did you see?<textarea value={tip} onChange={e => setTip(e.target.value)} required maxLength={5000}/></label><label>Approximate place and time<input value={location} onChange={e => setLocation(e.target.value)} maxLength={500}/></label><p>No contact details are requested here.</p><button className="sa-button" disabled={busy}>{busy ? 'Sending…' : 'Send private tip'}</button></form>}</section>}
      </section><aside className="sa-subscribe"><p className="sa-eyebrow">Stay aware, on your terms</p><h2>Follow an area</h2><p>Alerts reach registered browsers, not every phone nearby.</p><div className="sa-readiness"><span className={deviceReady && enrollment?.subscription_id ? 'sa-dot ready' : 'sa-dot'}/>{deviceReady && enrollment?.subscription_id ? 'Browser registered' : 'Notifications not confirmed'}</div>
        {!supported && <p className="sa-notice">This browser cannot enable push here. On iPhone or iPad, add this page to your Home Screen and open it there. You can always read the bulletin.</p>}
        {config && !config.enrollment_available && <p className="sa-notice">Notification enrollment is currently unavailable. The bulletin remains available.</p>}
        <label>Registered area<select value={area} onChange={e => setArea(e.target.value)}><option value="">Choose an area</option>{config?.areas.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></label>
        {config?.is_test && <label>Test invitation<input type="password" autoComplete="off" value={invite} onChange={e => setInvite(e.target.value)} placeholder="From your test organizer"/></label>}
        ...
        <button className="sa-button" disabled={busy || !consent || !area || !supported || !config?.enrollment_available} onClick={() => void enroll()}>{busy ? 'Working…' : enrollment?.subscription_id ? 'Update area / reconnect' : 'Enable notifications'}</button>
        {enrollment?.subscription_id && <button className="sa-stop" disabled={busy} onClick={() => void stop()}>Stop notifications</button>}
        <p className="sa-fine">Only public alert information appears in notifications. Permission does not guarantee delivery. Clearing this site's data removes these controls; browser settings can also block notifications.</p>
      </aside></div>
